...
import {
  ...
  GetAppealsUseCase,
} from '../../../use-cases/admin/AdminUseCases';
...
const setUserStatusUseCase =
  new SetUserStatusUseCase(
    userRepo,
    emailService
  );

const getAppealsUseCase =
  new GetAppealsUseCase(
    appealRepo
  );
...
export const getAppeals = async (
  ...
  try {
    const appeals =
      await getAppealsUseCase.execute();
  ...
  try {
    ...
    if (!adminMsg?.trim()) {
      return error(
        res,
        400,
        'A message to the user is required'
      );
    }

    const appeal =
      await appealRepo.updateStatus(
        appealId,
        status
      );
